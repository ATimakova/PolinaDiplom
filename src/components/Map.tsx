...
mapboxgl.accessToken = 'pk.eyJ1IjoiY2FyYWthem92IiwiYSI6ImNsNDFuY2tqNDA3bmQza2tjaWpraXkxZGcifQ.RXWBqGkSgaiwiAZcriLQ_Q'
...
const Map = () => {
	...
	useEffect(() => {
		if (role === 'ROLE_ADMIN') {
			...
			document.querySelectorAll('.mapbox-gl-show_add_form').forEach((el) => el.remove())
		}
	}, [role])
			...
			events.forEach((event: IEvent) => {
				...
				if (map) {
					...
					if (event.type === EventType.EVENT) {
						...
						date.innerHTML = `Дата: ${moment(event.date).format('DD.MM.YYYY HH:mm')}`
					...
					if (
						...
						(event.type === EventType.EVENT || event.type === EventType.ATTRACTION)
					) {
					...
					// create the popup
					const popup = new mapboxgl.Popup({ offset: 10 }).setDOMContent(htmlContent)
					// create the marker
	...
	return (
		<div className='map-container'>
			<div id='map'></div>
			{(editEvent || addEvent) && (
				<EditForm editData={editEvent} coordinates={coordinates} closeForm={closeForm}></EditForm>
			)}
			{event && <InformPopup eventId={event} closeForm={() => setEvent(null)}></InformPopup>}
		</div>
	)
}
