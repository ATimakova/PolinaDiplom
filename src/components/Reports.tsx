...
const Reports = () => {
	...
	return currentUser.role === 'ROLE_ADMIN' ? (
		...
		<div className='col-md-12'>Данная страница доступна только для администраторов!</div>
	)
}
...
const TableContent = ({ title, type }: { title: any; type: string }) => {
	...
	const getReports = (values: { from: string; to: string }) => {
		if (values.from && values.to) {
			ApiService.getEventsReport(currentUser.token, type, values.from, values.to).then(
